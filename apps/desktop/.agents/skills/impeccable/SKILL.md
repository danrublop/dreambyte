...
2. **Check .impeccable.md (fast)**: If not in instructions, read `.agents/.impeccable.md` (this repo keeps it there instead of the project root). If it exists and contains the required context, proceed.
...
Write this section to `.agents/.impeccable.md`. If the file already exists, update the Design Context section in place.

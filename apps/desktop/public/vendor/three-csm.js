var ThreeCSMLib = (() => {
  ...
  // ../../../private/tmp/three-csm-in.js
  var three_csm_in_exports = {};
  __export(three_csm_in_exports, {
    default: () => CSM
  });
  var import_three = window.THREE;
  var lightParsBeginInitial = import_three.ShaderChunk.lights_pars_begin;
  var CSMShader = {
...
uniform float shadowFar;
  ...
  var CSMHelper = class extends import_three.Group {
    constructor(csm) {
      super();
      ...
      const frustumGeometry = new import_three.BufferGeometry();
      frustumGeometry.setIndex(new import_three.BufferAttribute(indices, 1));
      frustumGeometry.setAttribute("position", new import_three.BufferAttribute(positions, 3, false));
      const frustumLines = new import_three.LineSegments(frustumGeometry, new import_three.LineBasicMaterial());
    ...
    update() {
      ...
      while (cascadeLines.length < cascades) {
        const cascadeLine = new import_three.Box3Helper(new import_three.Box3(), new import_three.Color(16777215));
        const planeMat = new import_three.MeshBasicMaterial({ transparent: true, opacity: 0.1, depthWrite: false, side: import_three.DoubleSide });
        const cascadePlane = new import_three.Mesh(new import_three.PlaneGeometry(), planeMat);
        const shadowLineGroup = new import_three.Group();
        const shadowLine = new import_three.Box3Helper(new import_three.Box3(), new import_three.Color(16776960));
  ...
  var inverseProjectionMatrix = new import_three.Matrix4();
  var CSMFrustum = class _CSMFrustum {
    constructor(data = {}) {
      this.vertices = {
        near: [
          new import_three.Vector3(),
          new import_three.Vector3(),
          new import_three.Vector3(),
          new import_three.Vector3()
        ],
  ...
  function practicalSplit(amount, near, far, lambda, target) {
    ...
    for (let i = 1; i < amount; i++) {
      target.push(import_three.MathUtils.lerp(_uniformArray[i - 1], _logArray[i - 1], lambda));
    }
    target.push(1);
  }
  var _origin = new import_three.Vector3(0, 0, 0);
  var _lightOrientationMatrix = new import_three.Matrix4();
  var _lightOrientationMatrixInverse = new import_three.Matrix4();
  var _cameraToLightParentMatrix = new import_three.Matrix4();
  var _cameraToLightMatrix = new import_three.Matrix4();
  var _lightSpaceFrustum = new CSMFrustum();
  var _center = new import_three.Vector3();
  var _bbox = new import_three.Box3();
  var _uniformArray = [];
  var _logArray = [];
  var CSM = class {
    constructor(data) {
      ...
      this.lightDirection = (_j = data.lightDirection) !== null && _j !== void 0 ? _j : new import_three.Vector3(1, -1, 1).normalize();
      this.lightDirectionUp = (_k = data.lightDirectionUp) !== null && _k !== void 0 ? _k : import_three.Object3D.DEFAULT_UP;
      this.lightIntensity = (_l = data.lightIntensity) !== null && _l !== void 0 ? _l : 1;
      this.lightColor = (_m = data.lightColor) !== null && _m !== void 0 ? _m : new import_three.Color(16777215);
    ...
    createLights() {
      for (let i = 0; i < this.cascades; i++) {
        const light = new import_three.DirectionalLight(this.lightColor, this.lightIntensity);
    ...
    injectInclude() {
      import_three.ShaderChunk.lights_fragment_begin = CSMShader.lights_fragment_begin(this.cascades);
      import_three.ShaderChunk.lights_pars_begin = CSMShader.lights_pars_begin(this.maxCascades);
    }
    ...
    getExtendedBreaks() {
      ...
      for (let i = 0; i < this.maxCascades; i++) {
        ...
        target.push(new import_three.Vector2(prev, amount));
      }
  ...
  return __toCommonJS(three_csm_in_exports);
})();

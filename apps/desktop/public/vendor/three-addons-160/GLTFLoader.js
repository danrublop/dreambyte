/**
 * GLTFLoader — vendored from three@0.160.0/examples/jsm/loaders/GLTFLoader.js.
 * Rewritten as a classic script that pulls THREE classes from window.THREE so
 * it shares the UMD THREE instance. Exposes window.GLTFLoader.
 *
 * The upstream file also imports `toTrianglesDrawMode` from
 * three/examples/jsm/utils/BufferGeometryUtils.js — we inline that function
 * below (it's ~70 LOC and has no further deps beyond THREE constants).
 *
...
;(function () {
  ...
  var T = window.THREE
  if (!T) {
    console.error('[GLTFLoader] window.THREE not found — load three.min.js first.')
    return
  }
  // Destructure every class GLTFLoader uses out of the UMD THREE global.
  ...
  /* --- Inlined from three/examples/jsm/utils/BufferGeometryUtils.js --- */
